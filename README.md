...
3. 試合日時（datetime-local）・場所・集合時間（time）を入力、定員/メモは任意 → 「試合を作成する」

- [ ] 管理者には「+ 試合を作成」ボタンが表示される
- [ ] 必須項目（日時・場所・集合時間）が空だとエラーが表示され送信されない
...
- [ ] 各試合をタップすると試合詳細が開き、日時・集合時間・場所・定員・メモが表示される
...
- [ ] 入力不備（例: `meetTime: "25:00"`）→ `400` とエラーメッセージ
