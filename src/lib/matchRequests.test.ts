...
test("validateMatchRequestInput: 希望場所・レベル感・参加予定人数・メモは任意（省略可）", () => {
  const r = validateMatchRequestInput({
    date: valid.date,
...
test("validateMatchRequestInput: 都道府県を受け付ける", () => {
  const r = validateMatchRequestInput({ ...valid, prefecture: "大阪府" })
  assert.ok(r.ok)
  assert.equal(r.value.prefecture, "大阪府")
})

test("validateMatchRequestInput: 都道府県が未指定/不正ならエラー", () => {
  for (const prefecture of [undefined, null, "", "Tokyo", "東京"]) {
    const r = validateMatchRequestInput({ ...valid, prefecture })
