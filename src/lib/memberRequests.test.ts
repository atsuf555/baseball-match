...
test("validateMemberRequestInput: ポジション・レベル感・メモは任意（省略可）", () => {
  const r = validateMemberRequestInput({
    count: 2,
    contactEmail: "contact@example.com",
    prefecture: "東京都",
  })
...
test("validateMemberRequestInput: 都道府県を受け付ける", () => {
  const r = validateMemberRequestInput({ ...valid, prefecture: "大阪府" })
  assert.ok(r.ok)
  assert.equal(r.value.prefecture, "大阪府")
})

test("validateMemberRequestInput: 都道府県が未指定/不正ならエラー", () => {
  for (const prefecture of [undefined, null, "", "Tokyo", "東京"]) {
    const r = validateMemberRequestInput({ ...valid, prefecture })
