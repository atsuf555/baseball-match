...
test("validateHelperRequestInput: ポジション・メモは任意（省略可）", () => {
  const r = validateHelperRequestInput({
    count: 2,
    contactEmail: "contact@example.com",
    prefecture: "東京都",
  })
...
test("validateHelperRequestInput: 都道府県を受け付ける", () => {
  const r = validateHelperRequestInput({ ...valid, prefecture: "大阪府" })
  assert.ok(r.ok)
  assert.equal(r.value.prefecture, "大阪府")
})

test("validateHelperRequestInput: 都道府県が未指定/不正ならエラー", () => {
  for (const prefecture of [undefined, null, "", "Tokyo", "東京"]) {
    const r = validateHelperRequestInput({ ...valid, prefecture })
