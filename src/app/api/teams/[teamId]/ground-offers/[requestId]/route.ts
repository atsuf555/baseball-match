...
// PATCH /api/teams/[teamId]/ground-offers/[requestId]
// グラウンド譲渡を締め切る（チームの管理者のみ）
export async function PATCH(
  request: Request,
  { params }: RouteContext<"/api/teams/[teamId]/ground-offers/[requestId]">
) {
  ...
  if (!groundOffer || groundOffer.teamId !== teamId) {
    return Response.json({ error: "グラウンド譲渡が見つかりません" }, { status: 404 })
  }
  ...
  const updated = await prisma.groundOffer.update({
    where: { id: requestId },
    data: { status: "CLOSED" },
    select: { status: true },
  })
